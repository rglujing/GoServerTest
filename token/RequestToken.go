...
func RequestToken (para string, addr string) (ret  string) {

	var reqpara requestToken_ParaReq
	...
	err, etnode := EntryControl(reqpara.Appid, addr, METHOD_REQUEST_TOKEN, true, reqpara.Secret)
	if err != nil {
		return Marshal_Response_Para(reqpara.ReqData, err)
	}

	ret, err = request_token( &reqpara, etnode )	
...
func request_token( req * requestToken_ParaReq, etnode * tokenetcd.Etcd_token_node) (ret string, err * TokenErr ) {
	...
	_, e = rand.Read(i)
	if e != nil {
		l4g.Error("request_token rand error:%s", e.Error())
		err = Err_Internal
		return
	}
